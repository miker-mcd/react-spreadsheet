...
class Cell extends Component {
  ...
  // Before updating, execute the formula on the Cell value to calculate the 'display' value. Especially useful when a redraw is pushed upon this cell when editing another cell that this might depend on
  componentWillUpdate() {
  ...
  // Handle pressing a key when the Cell is a span element, not yet in editing mode
  onKeyPressOnSpan = () => {
  ...
  // Called by the 'onBlur' or 'onKeyPressOnInput' event handlers, it escalates the value changed event, and restore the editing state to 'false'
  hasNewValue = value => {
  ...
  calculateCss = () => {
    const css = {
      ...
      lineHeight: '15px',
      overflow: 'hidden',
      fontFamily: "Calibiri, 'Segoe UI', Thonburi, Arial, Verdana, sans-serif"
    };
